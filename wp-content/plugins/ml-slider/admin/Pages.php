...
Class MetaSlider_Admin_Pages extends MetaSliderPlugin {
    ...
    public function add_page2($title, $title2,$slug = '', $parent = '') {
        $slug = ('' == $slug) ? sanitize_title($title) : $slug;
        $method = 'render_'.str_replace("-", "_", $slug).'_page';
        if (!method_exists($this, $method)) {
            return false;
        ...
        $page = ('' == $parent) ? add_menu_page($title, $title2, $capability, $slug, array($this, $method)) : add_submenu_page($parent, $title, $title2, $capability, $slug, array($this, $method));
